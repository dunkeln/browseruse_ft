...
def _held_out_values(values: list, count: int) -> set:
    unique = set(values)
    return set(
        sorted(unique, key=lambda value: sha256(str(value).encode()).digest())[:count]
    )
...
def load_task_split(split: str = "train", rows: int | None = None) -> Dataset:
    ...
    evaluation = load_dataset(
        "json", data_files=WEBARENA_LITE_EVAL_URL, split="train"
    )
    ...
    rft_tasks = all_tasks.select(
        [
            index
            for index, task_id in enumerate(all_tasks["task_id"])
            if task_id not in held_out_ids
        ]
    )
    ...
    if split == "train":
        dataset = rft_tasks.select(
            [
                index
                for index, task_id in enumerate(rft_tasks["task_id"])
                if task_id not in extra_test_ids
            ]
        )
        ...
        extra_test = rft_tasks.select(
            [
                index
                for index, task_id in enumerate(rft_tasks["task_id"])
                if task_id in extra_test_ids
            ]
        )
        evaluation = evaluation.add_column(
            "source_task_id", evaluation["old_task_id"]
        )
        evaluation = evaluation.add_column(
            "project_subset", ["official_eval"] * len(evaluation)
        )
        extra_test = extra_test.add_column(
            "source_task_id", extra_test["task_id"]
        )
        extra_test = extra_test.add_column(
            "project_subset", ["extra_holdout"] * len(extra_test)
        )
